# Annual reports SSA
## This do file cleans news on SSA DURING COVID19
...
# LOAD DATA ####
getwd()
load( file = "cr_data/news_covid_extracted.RData") 
...
covid <- str_extract_all(data$text, regex(pattern = "covid[1-9|\\-(1-9)]*", ignore_case = T), simplify = T) 
# because we have covid19 and covid-19 and covid

unique(covid) 
rm(covid)

data$text <- str_replace_all(data$text, 
                             regex(pattern = "covid[1-9|\\-(1-9)]*", ignore_case = T), 
                             replacement =  "covid")   
# replace variations with "covid"
...
unique(comm_hours)
tabyl(as_factor(comm_hours)) # 49 will be removed
...
str_view_all(data$text, regex(pattern = "andrew khoo", ignore_case = T), match = T ) 
...
# All the articles in this corpus (during covid) has ss/vwo/comm service phrases.
# Only in extracting sentences, i will include fsc/shelters etc -> see below
...
NROW(dup) # 39 duplicates in title

tidy1 <- tidy1[!duplicated(tidy1$term), ] # 1485-39 = 1446 
NROW(tidy1) # 1446 sentences
...
mean(tidy1_sentiment_nrc$valence)
...
tidy1_val_COVID <- tidy1_sentiment_nrc
#tidy1_val_COVID <- tidy1_sentiment_bing
#tidy1_val_COVID <- tidy1_sentiment_afinn
...
getwd()
save(tidy1_val_COVID, file =  "an_data/news_covid_valence" )

# SEANCE SENTIMENT ANALYSIS ####

## Prep data for conversion in SEANCE ======
tidy1_val_COVID_seance <- 
    tidy1_val_COVID %>% 
...
#getwd()
#for (i in 1:nrow(tidy1_val_COVID_seance)) {
#    write(tidy1_val_COVID_seance$term[i], 
#          paste0("an_data/seance/duringcovid/",
#                 tidy1_val_COVID_seance$id_seance[i], ".txt"))
#}
...
seance_duringcovid <- read_csv(file = "an_data/seance/output/results_duringcovid.csv")

seance_duringcovid$id_seance <- str_extract_all(seance_duringcovid$filename ,
                                                pattern = "[\\d]+", simplify = T)

seance_duringcovid$id_seance <- as.integer(seance_duringcovid$id_seance)
...
unique(seance_duringcovid$id_seance)
    # very strange! there are duplicates

dup <- seance_duringcovid[duplicated(seance_duringcovid$id_seance), ] 
NROW(dup) # 3 duplicates in id

seance_duringcovid <- seance_duringcovid[!duplicated(seance_duringcovid$id_seance), ] # 1449-3 = 1446
NROW(seance_duringcovid)

## Merge
seance_duringcovid <- 
    seance_duringcovid %>% 
    select(-filename) %>% 
    right_join(tidy1_val_COVID_seance) %>% 
    mutate(covid = 1) %>% 
    relocate(document, term, id_sentence, id_seance, covid, .before =nwords)  
    

glimpse(seance_duringcovid)

save(seance_duringcovid, file = "an_data/seance_duringcovid")
